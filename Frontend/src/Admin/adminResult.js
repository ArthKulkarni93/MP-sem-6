...
const AdminResult = () => {
  ...
  // Check if the test is complete by comparing current time to scheduled time + duration
  const isTestComplete = (test) => {
    const scheduled = new Date(test.scheduledDate);
    const completeTime = new Date(scheduled.getTime() + test.duration * 60000);
    return new Date() >= completeTime;
  ...
  // Fetch the results for a given test and display them
  const handleViewResults = async (test) => {
    try {
      const token = localStorage.getItem('token');
      const endpoint = api.getTestResults.url.replace(':testid', test.id);
  ...
  // When clicking on the "Yes" button for cheated results,
  // build the cheat details object directly from the result object and display it.
  const handleViewCheatDetails = (result) => {
    const details = {
      maxFaceCount: result.maxFaceCount,
      tabSwitchCount: result.tabSwitchCount,
      // Rename 'fullScreenExits' to 'fullScreenEscaped' for UI consistency
      fullScreenEscaped: result.fullScreenExits,
    };

    setCheatDetails(details);
    setModalOpen(true);
    console.log("Cheat details", details);
  };
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-30">
            <div className="bg-white p-6 rounded shadow-lg w-80">
              ...
              <table className="min-w-full mb-4">
                <tbody>
                  <tr>
                    <td className="py-2 px-3 font-medium border">Max Face Count</td>
                    <td className="py-2 px-3 border">{cheatDetails.maxFaceCount}</td>
                  </tr>
                  <tr>
                    <td className="py-2 px-3 font-medium border">Tab Switch Count</td>
                    <td className="py-2 px-3 border">{cheatDetails.tabSwitchCount}</td>
                  ...
                  <tr>
                    <td className="py-2 px-3 font-medium border">Full Screen Escaped</td>
                    <td className="py-2 px-3 border">{cheatDetails.fullScreenEscaped}</td>
                  </tr>
                </tbody>
              </table>
              <button 
